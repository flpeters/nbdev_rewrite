...
__all__ = ['iter_comments', 'update_from_all_', 'fastai_patch', 'find_names', 'find_default_export', 'not_private', 'read_nb', 'add_names_A', 'unwrap_attr', 'kw_export', 'OptionsTuple', 'legacy_parse_options', 'ExportCache', 'notebook2script', 'add_names_FC', 'keyword_parser', 'create_mod_file', 'relative_import', 'kw_hide', 'find_exports', 'KeywordParser', 'parse_options', 'update_recursive']
...
# Cell
def update_from_all_(node, names): # TODO: should all of these cases be handled, or just always expect a string?
...
def add_names_A(node, names):
    ...
    for name in tmp_names:
        ...
        # NOTE: cases below can only use private variable names
            ...
            if len(tmp_names) != 1: raise SyntaxError('reserved keyword _all_ can only be used in simple assignments')
...
# Cell
def add_names_FC(node, names, fastai_decorators=True):
    if fastai_decorators and ('patch' in [d.id for d in node.decorator_list]):
        if not (len(node.args.args) >= 1): raise SyntaxError('fastai\'s @patch decorator requires at least one parameter')
        cls = node.args.args[0].annotation
        if cls is None: raise SyntaxError('fastai\'s @patch decorator requires a type annotation on the first parameter')
...
def find_names(code:str) -> list:
    ...
    for node in tree.body:
        if   isinstance(node, _ast.Assign): add_names_A(node, names)
        elif isinstance(node, (_ast.FunctionDef, _ast.ClassDef)): add_names_FC(node, names)
        else: pass
    return names
