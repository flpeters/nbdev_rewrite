...
__all__ = ['CellInfo', 'Config', 'DictLikeAccess', 'DictLikeRepr', 'ExportUnit', 'FileInfo', 'MODULE__MAIN__FLAG', 'MergedFileInfo', 'ScopeUnit', 'StackTrace', 'Traced', 'all_commands', 'async_load_notebooks', 'cmd2func', 'crawl_directory', 'create_config', 'find_names', 'from_string_cell', 'init_lib', 'iter_comments', 'kw_default_exp', 'kw_export', 'main', 'make_valid_path', 'merge_all', 'module_to_path', 'parse_all', 'parse_comment', 'parse_file', 'read_nb', 'register_command', 'relativify_imports', 'report_successful_export', 'set_main_report_options', 'write_all', 'write_file']
...
# Cell nr. 227
class ScopeUnit(DictLikeAccess, DictLikeRepr):
...
class FileInfo(DictLikeAccess, DictLikeRepr):
    ...
    def __init__(self, origin_file, nb_version,
        ...
        self.relative_origin = os.path.relpath(origin_file, Config().proj_path).replace('\\', '/')
...
# Cell nr. 231
class MergedFileInfo(DictLikeAccess, DictLikeRepr):
...
def kw_export(file_info, cell_info, result, is_set, st:StackTrace) -> bool:
    ...
    success:bool = True
    export_unit:ExportUnit = ExportUnit(cell_info)
...
def crawl_directory(path:Path, recurse:bool=True) -> list:
    ...
    _reserved_dirs = (Config().lib_path, Config().doc_path) # TODO: Don't recreate this every time.
...
def merge_all(parsed_files:dict, st:StackTrace) -> (bool, dict):
    success:bool = True
    merged_files = defaultdict(MergedFileInfo)
    zero_tuple = (0,)
    ...
    for file_info in parsed_files:
        ...
        default_scope   :ScopeUnit = scopes[zero_tuple]
...
def main(recurse:bool=True, st:StackTrace=None) -> (bool, dict, dict):
    ...
    # NOTE: LOAD
    notebooks = async_load_notebooks(path=Config().nbs_path, recurse=recurse)
